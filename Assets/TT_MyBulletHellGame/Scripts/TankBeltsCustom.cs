...
public class TankBeltsCustom : MonoBehaviour
{
    [FormerlySerializedAs("bot")] [SerializeField]
	...
	{
		float tankMovementMultiplier = tankBot.navAgent.velocity.magnitude;

		// Only adjust the treads if the tank is moving
		if (tankMovementMultiplier > 0.05f)
		{
			// Which direction is the tank moving?
			float dot = Vector3.Dot(tankBot.navAgent.velocity, _treadTransforms[treadIndex].forward);
